...
namespace EmoEngineClientLibrary
{
    ...
    public class EmoEngineClient : INotifyPropertyChanged, IDisposable
    {
        ...
        /*
         *  FUNCTION THAT WRITES THE INCOMING DATA
         */
        void ESUM_LogData(Dictionary<EdkDll.EE_DataChannel_t, double[]> currentData)
        {
            if (logData)
            {

                if (ESUM_Util.exportFolder == null)
                {
                    ESUM_Util.CreateDirectory();
                }
                ...
                //add timestamp
                sb.Append(ESUM_Util.GetTimeStamp());
                //convert data readings to string

                //string channelDat = string.Join(",", currentData.Select(x => x.Key + "=" + x.Value).ToArray());
                //string doubleVals = string.Join(",", arr.Select(p => p.ToString()).ToArray());
                string channelDat = string.Join(",", currentData.Select(x => x.Key + "> ," + string.Join(",", x.Value.Select(p => p.ToString()).ToArray())).ToArray());
                sb.Append(channelDat);
                sb.Append(ESUM_Util.newLine);

                //iterate counter
                dataCounter++;
                if (dataCounter == maxFramesToSave)
                {
                    //export data
                    ESUM_Util.ExportText("EEG", sb.ToString());
                    //clear stringBuilder and reset counter
                    dataCounter = 0;
                    sb.Clear();
                }
            }
        ...
        int nDataChannels = Enum.GetNames(typeof(EdkDll.EE_DataChannel_t)).Length;
        bool logData = true;
        const int maxFramesToSave = 128;
        int dataCounter = 0;
        StringBuilder sb;
